...
public partial class TribeMember
{
   ...
   public void hookpunsh_StartSwing()
    {
    ...
    public void hookpunsh_Hit()
    {
        if (!hasHitThisSwing)
        {   attackRadius=0.2f;
            // Pass the current values dynamically!
            // Range: 1.5f, Angle: 60f, Damage: 20f, Twist: currentTwistAngle
            PerformHitboxCheck(weaponHandAnchorright,attackRadius,20f);
            hasHitThisSwing=true;
        }
    }
    
    public void hookpunsh_EndHit()
    {
        currentTwistAngle = 0f;
        isMidSwing = false;
        ...
        // 2. ENABLE THE FLAG
        isanimationPosition = true;
        
    }

    private void hookpunsh()
    {
        ...
        if (actionCooldown >= actionRate)
        {
            ...
            if (anim != null)
            {
                ...
                anim.SetFloat("Hook Punchspead", animSpeed);
                anim.SetTrigger("Hook Punch"); // This fires AnimEvent_StartSwing -> AnimEvent_Hit
            }
        }
    }
